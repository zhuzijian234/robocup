 ...
 *   例: {"kp", ..., 3} → 手机发 "kp 500" 实际写入 500/1000 = 0.5
 * min_i/max_i 是"发送值"的合法范围, 超范围回 ERR range 且不写值 */
typedef struct {
    const char *name;   /* 命令名 */
    float      *ptrs[4];/* 被调参数指针(可多个同写, 如 kp → 曲线R_CL_kp+L_CL_kp) */
    uint8_t     n;      /* 指针个数 */
    int16_t     min_i;  /* 发送值下限 */
    int16_t     max_i;  /* 发送值上限 */
...
static const Param_t param_tab[] = {
    /* 谢露舵机PD: 千分位 (scale=3), 双指针同写(曲线/直角各一对) */
    {"kp",    {&R_CL_kp, &L_CL_kp},   2, 0, 1000, 3},   /* 曲线kp   发500 → 0.5 */
    {"kp2",   {&YZ_j_kp, &ZZ_j_kp},   2, 0, 1000, 3},   /* 直角kp   发900 → 0.9 */
    {"kd",    {&R_CL_kd, &L_CL_kd},   2, 0, 1000, 3},   /* 曲线kd   发300 → 0.3 */
    {"kd2",   {&YZ_j_kd, &ZZ_j_kd},   2, 0, 1000, 3},   /* 直角kd   发130 → 0.13 */
    /* 电机PI: kp×100, ki×1000 */
    {"sp_kp", {&Speed_pid.kp},        1, 0, 5000, 2},   /* 发850 → 8.5 */
    {"sp_ki", {&Speed_pid.ki},        1, 0, 5000, 3},   /* 发505 → 0.505 */
    {"spd",   {&Speed_mubiao},        1, 0,   50, 0},   /* 目标速度 1:1 */
    /* 谢露规划阈值: 1:1 (scale=0) */
    {"zd",    {&zhuandian},           1, 0, 1000, 0},   /* 断点阈值     发200 → 200 */
    {"dd",    {&Duandian_d},          1, 0, 2000, 0},   /* 墙点y带通上限 发700 → 700 */
};
...
/* 每雷达帧调用一次: 7通道 = 误差/舵机PWM/活动kp/活动kd/pid模式(1-4,0=未决策)/当前速度/目标速度
...
void BLE_Tune_Telemetry(float err, float servo_pwm, uint16_t pid_mode)
{
    ...
    XIELU_ActiveGains(&act_kp, &act_kd);            /* 按当前pid_slt取活动增益(曲线/直角) */
    Tele_SendFloat(act_kp);
    Tele_SendFloat(act_kd);
...
/* ======================== 【调参】get 命令回显 ======================== */
/* 回发全部参数 "名字=整数\r\n", 整数按 scale 四舍五入 (如 kp → "kp=500")。
 * 注意: 9行 ≈ 130字节 @9600 ≈ 135ms, 主循环会停控约0.14秒, 建议停车时用 */
static void Tune_SendAll(void)
{
    ...
    for (i = 0; i < PARAM_NUM; i++) {
        int iv = (int)(*param_tab[i].ptrs[0] * mul[param_tab[i].scale] + 0.5f);
        sprintf(line, "%s=%d\r\n", param_tab[i].name, iv);
...
static void Tune_ApplyOne(char *line)
{
    ...
    *sp = '\0';                     /* 切断 → line="kp", sp+1="45..." */
    ...
    for (i = 0; i < PARAM_NUM; i++) {
         ...
         * 绝不调用 Speed_PID_Init —— 它会清零误差状态 */
        for (j = 0; j < param_tab[i].n; j++)
            param_tab[i].ptrs[j][0] = (float)iv / mul[param_tab[i].scale];
        sprintf(ack, "OK %s=%d\r\n", line, iv);
