...
/**
 * @brief  初始化USART3的GPIO引脚 (TX=PB10, RX=PB11)
...
/*参数3: BSP_BLUETOOTH_AF
    ...
    RCC_APB1PeriphClockCmd(BSP_BLUETOOTH_RCC, ENABLE); /*开启USART3时钟挂载的APB1总线的时钟*/
    USART_DeInit(BSP_BLUETOOTH);/*把USART3的寄存器配置恢复到默认状态*/
    ...
    USART_ITConfig(BSP_BLUETOOTH, USART_IT_RXNE, ENABLE); /*每收到一个字节,触发usart3中断*/
    USART_ITConfig(BSP_BLUETOOTH, USART_IT_IDLE, ENABLE); /*硬件自动检测(看高电平),总线空闲,触发usart3中断*/
    /*清理残留中断标志*/
    USART_ClearFlag(BSP_BLUETOOTH, USART_FLAG_RXNE);
    USART_ClearFlag(BSP_BLUETOOTH, USART_IT_IDLE);
    USART_Cmd(BSP_BLUETOOTH, ENABLE);
    /*把 USART3->CR1 的 UE（USART Enable）bit 置 1。在此之前 USART3 配置完毕但不工作，这行一调，正式开始收发。*/
    /* NVIC中断优先级配置 */
...
/**
 * @brief  蓝牙完整初始化: GPIO + USART3 + STATE引脚
 */
...
/**
 * @brief  USART3中断服务函数(ISR) — 蓝牙数据接收
 *重写默认的USART3_IRQHandler()函数,实现蓝牙数据接收
