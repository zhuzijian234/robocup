...
void Ano_Anl(uint8_t *Data_Pack)          /* 数据解析 */
{
    ...
    if(Data_Pack[2] == 0xE1) /* 参数读取ID 0XE1 */
    {
        ...
        {            
            case 1: 
                Ano_SentPar(id,(Speed_pid.kp)*10); /* Kp放大10倍发送 */
            break;
            case 2:
                Ano_SentPar(id,Speed_pid.ki*10); /* Ki放大10倍发送 */
            break;
            case 3:
                Ano_SentPar(id,Speed_pid.kd*10); /* Kd放大10倍发送 */
            break;
            case 4:
                Ano_SentPar(id,Speed_mubiao/10);     //所有转速都是除10         /* 转速  */
            break;
    ...
    if(Data_Pack[2] == 0xE2) /* 参数写入ID 0XE2，接收到的是整数*/
    {
        ...
        {
            case 1:
                Speed_pid.kp=(float)*(int *)(&Data_Pack[6])/10;
            break;
            case 2:
                Speed_pid.ki=(float)*(int *)(&Data_Pack[6])/10;
            break;
            case 3:
                Speed_pid.kd=(float)*(int *)(&Data_Pack[6])/10;
            break;
            case 4:
                Speed_mubiao=(float)*(int *)(&Data_Pack[6])*10; //转速限幅
            break;                 
        }
    }
